...
extern int g_have_encoding_frame_count;
int postProcessSSD(float * predictions, float *output_classes, int width,
                   int heigh,unsigned char* in_data)
{
	...
	x1 = max(x1,0);
        ...
        if (x1 == 0 && x2 == 0 && y1 == 0 && y2 == 0)
            continue;
	char name[100];
	sprintf(name,"%s__%s__%d__%d_%d_%d_%d",g_filename,labels[topClassScoreIndex],g_have_encoding_frame_count,x1,y1,x2,y2);
        printf("name:%s \n",name);
	SaveImage(name,in_data,300,300);
    }
...
int run_ssd(char* in_data)
{
  ...
  //   printf("camera callback w=%d h=%d c=%d\n", w, h, c);
    ...
    postProcessSSD(output1, output0, w, h,in_data);
    return 0;
}
