...
int GetBaseInfoToAck(WORD flowId, BYTE functionCode)
{
    ...
    dataArea[0] = 6;
    strncpy((char *)&dataArea[1], "nopluz", 6);
    
    dataArea[7] = 3;
    strncpy((char *)&dataArea[8], "BSD", 3);

    dataArea[11] = 5;
    strncpy((char *)&dataArea[12], "1.1.1", 5);

    dataArea[17] = 5;
    strncpy((char *)&dataArea[18], "2.2.2", 5);

    dataArea[23] = 5;
    strncpy((char *)&dataArea[24], "88888", 5);

    dataArea[29] = 5;
    strncpy((char *)&dataArea[30], "66666", 5);


    commuParam.isSendAckFlag = 1;

    SendRequestAck(flowId, functionCode, dataArea, 35);
