...
extern int global_socket_write;
                   ...
                   int heigh, struct ssd_group *group)
{
...
#if 1
	char data_temp[512];
	memset(data_temp,0,512);
    	if(global_socket_write && group->count){
	sprintf(data_temp,"{COUNT:%d",group->count);
	for (int i = 0; i < group->count; ++i) {
			sprintf(temp,",(%s,%d,%d,%d,%d)",group->objects[i].name, group->objects[i].select.left*640/300,group->objects[i].select.top*480/300, group->objects[i].select.right*640/300, group->objects[i].select.bottom*480/300);
			string_length += strlen(temp);
			strcat(data_temp,temp);
	}
	strcat(data_temp,"}");
        if(send(connect_fd, data_temp, strlen(data_temp),0) == -1){  
        	printf("send error");  
        	close(connect_fd);  
		global_socket_write = 0;
	}
	...
	memset(temp,0,128);
        ...
        if(send(connect_fd, temp, strlen(temp),0) == -1){  
                printf("send error");  
                close(connect_fd);  
                global_socket_write = 0;
        }
